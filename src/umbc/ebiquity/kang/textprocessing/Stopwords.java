...
public class Stopwords {
	...
	private String adj_sw = projectDir + "/Stopwords/ADJ.TXT";
	private String prep_sw = projectDir + "/Stopwords/PREP.TXT";
	private String verbs_sw = projectDir + "/Stopwords/VERBS.TXT";
	private String mine_sw = projectDir + "/Stopwords/MINE.TXT";
	...
	static {
		if (m_Stopwords == null) {
			m_Stopwords = new Stopwords(true);
		}
	}
	 ...
	 * target="_blank">Rainbow</a>).
	 */
	public Stopwords(boolean addDefault) {
...
//			add("off");
	...
	public void read() throws Exception {
		// clear();
		// read(new File(adj_sw));
		// read(new File(prep_sw));
		// read(new File(verbs_sw));
		read(new File(mine_sw));
	}
	...
	public void printStopwords() {
		for (String stopword : m_Words) {
			System.out.println(stopword);
		}
		System.out.println("Number of Stopwords " + m_Words.size());
	}

	public static void main(String[] argv) {
		try {
			Stopwords sw = new Stopwords(true);
			sw.read();
			sw.printStopwords();
		} catch (Exception ioe) {
			ioe.printStackTrace();
